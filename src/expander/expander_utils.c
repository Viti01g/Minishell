...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/26 10:58:36 by vruiz-go          #+#    #+#             */
/*   Updated: 2024/04/26 10:58:37 by vruiz-go         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*create_and_fill_array(char *input)
{
	...
	while (input[i[0]])
	{
		if (input[i[0]] == '\'')
			in_quotes = !in_quotes;
		handle_var_expand(input, i, result, &in_quotes);
	}
	result[i[1]] = '\0';
	free(input);
	return (result);
...
void	handle_var_expand(char *input, int *i, char *result, int *in_quotes)
{
	...
	if (input[i[0]] == '$' && !(*in_quotes)
		&& (input[i[0] + 1] == '_' || ft_isalpha(input[i[0] + 1])))
	{
		var_start = i[0];
